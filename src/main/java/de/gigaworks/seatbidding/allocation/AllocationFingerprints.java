...
final class AllocationFingerprints {
    ...
    /*
     * Encoding v2 is UTF-8, line-oriented, and uses only stable numeric identifiers,
    ...
    static String input(RoundAllocation.Problem problem) {
        var value = new StringBuilder("seat-bidding-allocation-input|v2\n")
        ...
        for (var date : problem.dates()) {
            value.append("date|").append(date.dateId()).append('|').append(date.targetDate()).append('|')
                    .append(date.capacity()).append('|').append(date.unresolvedSeats()).append('\n');
    ...
    static String solution(java.util.List<RoundAllocation.Result> results) {
        var value = new StringBuilder("seat-bidding-allocation-solution|v2\n");
