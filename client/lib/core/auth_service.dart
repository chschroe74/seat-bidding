...
class AuthService extends ChangeNotifier {
  ...
  static Future<AuthService> initialize() async {
    ...
    if (!kIsWeb) {
      ...
      if (nativeBase == null ||
          nativeBase.scheme != 'https' ||
          !nativeBase.hasAuthority ||
          !nativeBase.path.endsWith('/api')) {
        throw StateError(
          'Android requires an HTTPS API_BASE_URL ending in /api.',
        );
      }
    }
    ...
    dio.interceptors.add(
      InterceptorsWrapper(
        onRequest: (options, handler) {
          if (service.csrfToken != null &&
              _changesState(options.method) &&
        ...
        onError: (error, handler) async {
          final isLogin = error.requestOptions.uri.path.endsWith(
            '/j_security_check',
          );
  ...
  Future<void> login(String password) async {
    ...
    if (currentEmail == null) {
      throw StateError('The email-first login flow has not been started.');
    }
  ...
  Future<void> resend() async {
    ...
    try {
      ...
      if (error.response?.statusCode == 429) {
        _applyRetryAfter(error.response?.headers.value('retry-after'));
      }
      rethrow;
    }
  }
  ...
  void rememberIntendedRoute(String route) {
    if (route == '/assignments' || route == '/bids' || route == '/help') {
